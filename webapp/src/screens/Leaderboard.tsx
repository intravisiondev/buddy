...
import { leaderboardService, userService, friendService } from '../services';
...
export default function Leaderboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [lb, stats, myBadges, friendsRes, incomingRes] = await Promise.all([
        leaderboardService.getLeaderboard(period).catch(() => []),
        userService.getMyStats().catch(() => null),
        leaderboardService.getMyBadges().catch(() => []),
        friendService.getFriends().catch(() => []),
        friendService.getIncomingRequests().catch(() => []),
      ]);
      setEntries(Array.isArray(lb) ? (lb as LeaderboardEntry[]) : []);
      setMyStats(stats);
      setMyBadgesRaw(Array.isArray(myBadges) ? myBadges : []);
      const flist = Array.isArray(friendsRes) ? friendsRes.map((f: any) => f.friend_id || f.user_id) : [];
      setFriends(flist);
  ...
  // Resolve names for incoming requests
  ...
  useEffect(() => {
    const resolve = async () => {
      ...
      for (const r of incomingRequests) {
        ...
        if (r.from_user_name) {
          map[id] = r.from_user_name;
        } else if (entryMap[id]) {
          map[id] = entryMap[id];
        } else {
          try {
            const p: any = await userService.getUserProfile(id);
            map[id] = p?.name || `User ${id.slice(-6)}`;
          } catch {
  ...
  const earnedBadges = useMemo(() => safeBadgesRaw.map(b => b?.badge || b).filter(Boolean), [safeBadgesRaw]);
    ...
    <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-3 gap-6">
          {topRankers.map((u) => (
            <Card 
              ...
              className="p-6 bg-gradient-to-br from-light-bg to-light-card dark:from-dark-bg dark:to-dark-card cursor-pointer hover:scale-[1.02] transition-transform"
              onClick={() => {
      ...
      <div className="grid grid-cols-3 gap-8">
        <div className="col-span-2">
          <Card className="p-0">
            <div className="p-6 border-b border-light-text-secondary/10 dark:border-dark-border flex items-center justify-between">
              <h3 className="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary">
                Rankings
              </h3>
              <Badge variant="neutral" size="sm">{period === 'weekly' ? 'Weekly' : 'All-time'}</Badge>
            </div>
            <div className="divide-y divide-light-text-secondary/10 dark:divide-dark-border">
              {safeEntries.map((u) => (
                <div 
                  key={normalizeId(u.user_id)} 
                  className="p-4 flex items-center justify-between hover:bg-light-bg dark:hover:bg-dark-bg transition-colors cursor-pointer"
                  onClick={() => {
                    setSelectedUserId(normalizeId(u.user_id));
                    setSelectedUserName(u.name);
                    setSelectedUserAvatar(u.avatar_url);
            ...
            {earnedBadges.length === 0 ? (
              ...
              <div className="space-y-3">
                {earnedBadges.slice(0, 6).map((b: any) => (
                  <div key={b.id} className="p-3 rounded-lg bg-light-bg dark:bg-dark-bg">
              ...
              <div className="space-y-2">
                {friends.map((fid) => {
                  ...
                  return (
                    <div
                      key={fid}
                      className="flex items-center justify-between p-2 rounded-lg hover:bg-light-bg dark:hover:bg-dark-bg cursor-pointer"
                      onClick={() => {
            ...
            {incomingRequests.length === 0 ? (
              ...
              <div className="space-y-3">
                {incomingRequests.map((r) => {
                  ...
                  const name = incomingNames[fid] || r.from_user_name || `User ${fid.slice(-6)}`;
                  return (
                    <div
                      key={rid}
