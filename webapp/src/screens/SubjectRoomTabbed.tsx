...
export default function SubjectRoomTabbed() {
  ...
  const loadRoomData = async () => {
    setLoadingRoom(true);
    try {
      const room = await roomService.getRoom(selectedRoom!);
  ...
  const checkMembership = async () => {
    try {
      const members = await roomService.getRoomMembers(selectedRoom!);
  ...
  const handleJoin = async () => {
    setJoining(true);
    try {
      await roomService.joinRoom(selectedRoom!);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto bg-light-bg dark:bg-dark-bg">
        ...
        {activeTab === 'chat' && isMember && selectedRoom && (
          <SubjectRoomChat roomId={selectedRoom} />
        )}

        {activeTab === 'resources' && isMember && selectedRoom && (
          <ResourcesTab roomId={selectedRoom} />
        )}

        {activeTab === 'games' && isMember && selectedRoom && (
          <GamesTab roomID={selectedRoom} isTeacher={user?.role === 'teacher'} />
        )}

        {activeTab === 'ai-coach' && isMember && selectedRoom && (
