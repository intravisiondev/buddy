...
import { TrendingUp, Clock, BookOpen, Trophy, Target, Calendar, UserPlus, Loader } from 'lucide-react';
...
export default function ParentDashboard() {
  ...
  const loadChildren = async () => {
    ...
    try {
      ...
      const childrenData = await userService.getChildren().catch(() => []);
  ...
  const loadChildData = async (childId: string) => {
    try {
      ...
      // Get child stats
      const stats = await userService.getUserStats(childId).catch(() => null);
  ...
  const handleAddChild = async (data: {
    ...
    try {
      const { userService } = await import('../services');
      await userService.createChildAccount(data);
