...
export default function GamePlayer({ isOpen, onClose, game, onGameComplete }: GamePlayerProps) {
  ...
  const loadGame = async () => {
    ...
    setLoading(true);
    try {
      // Download and extract game bundle; returns HTTP URL for iframe
      const gameUrl = await gameService.downloadGameBundle(game.id);
  ...
  const handleGameEnd = async (result: any) => {
    ...
    try {
      ...
      const gameResult = await gameService.playGame(
        game.id,
