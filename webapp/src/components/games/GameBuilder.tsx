...
import { Gamepad2, Wand2, Settings } from 'lucide-react';
...
export default function GameBuilder({ isOpen, onClose, roomID, onGameCreated }: GameBuilderProps) {
  ...
  const loadTemplates = async () => {
    try {
      const data: any = await gameService.getGameTemplates();
  ...
  const getComplexityColor = (complexity: string): 'primary' | 'success' | 'warning' | 'error' | 'accent' | 'neutral' => {
    switch (complexity) {
