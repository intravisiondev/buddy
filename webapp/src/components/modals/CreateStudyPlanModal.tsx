...
import { Plus, Calendar, Flag, Globe, Clock, Target } from 'lucide-react';
...
export default function CreateStudyPlanModal({ isOpen, onClose, onSuccess }: CreateStudyPlanModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create study plan
      const planResult: any = await studyPlanService.createStudyPlan({
      ...
      if (autoCreateSchedule && scheduleBlocks.length > 0) {
        for (const block of scheduleBlocks) {
          try {
            await studyPlanService.createScheduleBlock(planID, block);
          } catch (err: any) {
      ...
      if (autoCreateMilestone && milestones.length > 0) {
        for (const milestone of milestones) {
          try {
            ...
            await goalService.createMilestone({
