...
export default function StopStudySessionModal({
  ...
  const loadMilestones = async () => {
    ...
    try {
      // GetMilestones expects a string
      const data: any = await goalService.getMilestones(studyPlanID);
  ...
  const handleStep1Continue = async () => {
    ...
    try {
      const response: any = await aiService.generateAssessmentQuestions(subject, notes || '', durationMinutes);
