...
import { BookOpen, Users, FileText, BarChart, Plus, Settings } from 'lucide-react-native';
...
export default function TeacherDashboard() {
  ...
  const stats = [
    { label: 'Classrooms', value: '0', icon: BookOpen, color: colors.primary, bg: 'rgba(99,102,241,0.12)' },
    { label: 'Students', value: '0', icon: Users, color: colors.success, bg: 'rgba(16,185,129,0.12)' },
    { label: 'Assignments', value: '0', icon: FileText, color: colors.accent, bg: 'rgba(139,92,246,0.12)' },
    { label: 'Reports', value: '0', icon: BarChart, color: colors.warning, bg: 'rgba(245,158,11,0.12)' },
  ];

  const actions = [
    { label: 'Create Classroom', icon: BookOpen, color: colors.primary },
    { label: 'Create Assignment', icon: FileText, color: colors.success },
    { label: 'View Analytics', icon: BarChart, color: colors.accent },
  ];
  ...
  return (
    <ScrollView style={s.screen} contentContainerStyle={s.content}>
      ...
      <View style={s.headerRow}>
        ...
        <Avatar name={user?.name || 'Teacher'} src={user?.avatar} size="lg" />
      </View>
      ...
      {/* Classrooms */}
      <Text style={s.sectionTitle}>My Classrooms</Text>
      <Card style={s.emptyCard}>
        <View style={s.emptyInner}>
          <View style={[s.emptyIconWrap, { backgroundColor: 'rgba(99,102,241,0.1)' }]}>
            <BookOpen size={32} color={colors.primary} />
          </View>
          <Text style={s.emptyTitle}>No classrooms yet</Text>
          <Text style={s.emptyText}>Create your first classroom to get started</Text>
