...
import { GetUserSchedule, GetMyStudyPlans, UpdateScheduleBlock, SaveTextToDownloads } from '../../../wailsjs/go/main/App';
...
export default function ScheduleTab() {
  ...
  const onDropOnDay = (newDay: number) => async (e: React.DragEvent) => {
    ...
    try {
      ...
      await UpdateScheduleBlock(payload.study_plan_id, payload.id, { day_of_week: newDay });
  ...
  const handleExportICS = async () => {
    try {
      ...
      const path = await SaveTextToDownloads('buddy-schedule.ics', ics);
      console.log('ICS saved to:', path);
    } catch (error) {
