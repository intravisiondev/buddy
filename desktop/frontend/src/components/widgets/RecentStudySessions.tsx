...
import { GetMyActivity, GetMyBadges } from '../../../wailsjs/go/main/App';
// @ts-ignore
...
export default function RecentStudySessions({ limit = 5 }: { limit?: number }) {
  ...
  const load = async () => {
    ...
    try {
      const [data, badges] = await Promise.all([
        GetMyActivity('study', Math.max(50, limit * 10)),
        GetMyBadges().catch(() => []),
      ]);
      setItems(Array.isArray(data) ? (data as ActivityLog[]) : []);
  ...
  useEffect(() => {
    load();
    EventsOn('activity:studySessionLogged', () => load());
    return () => {
