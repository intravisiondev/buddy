...
import { GetRoomAnalytics, ExportAnalyticsCSV } from '../../../wailsjs/go/main/App';
...
export default function GameAnalyticsDashboard({ roomID }: GameAnalyticsDashboardProps) {
  ...
  const loadAnalytics = async () => {
    setLoading(true);
    try {
      const data: any = await GetRoomAnalytics(roomID);
      setAnalytics(data);
    } catch (err) {
  ...
  const handleExportCSV = async () => {
    setExporting(true);
    try {
      await ExportAnalyticsCSV(roomID);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-6 hover:shadow-lg transition-shadow">
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge variant="secondary" size="sm">Avg</Badge>
          </div>
          <div className="text-3xl font-bold text-light-text-primary dark:text-dark-text-primary mb-1">
            ...
            {analytics.top_games.map((game: GamePerformance, index: number) => (
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-right">
                    <Badge
                      variant={game.pass_rate >= 70 ? 'success' : game.pass_rate >= 50 ? 'warning' : 'danger'}
                    >
