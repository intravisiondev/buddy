...
export default function TeacherDashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // @ts-ignore
      const { GetMyRooms, GetDashboardStats, GetAssignments, GetRoomMembers } = await import('../wailsjs/go/main/App');

      // Load classrooms (rooms)
      const rooms = await GetMyRooms('').catch(() => []);
      ...
      const membersPromises = (Array.isArray(rooms) ? rooms : []).map(async (room: any) => {
        const members = await GetRoomMembers(room.id).catch(() => []);
