...
import { GetMyStudyPlans, UpdateStudyPlanProgress, StartStudySession, GetMyActivity } from '../../wailsjs/go/main/App';
...
export default function StudyPlanDetail() {
  ...
  const loadPlanDetails = async () => {
    try {
      ...
      const allActivities: any = await GetMyActivity('study', 200);
  ...
  const handleUpdateProgress = async (newProgress: number) => {
    ...
    try {
      ...
      await UpdateStudyPlanProgress(plan.id, p);
  ...
  const handleStartStudy = async () => {
    ...
    try {
      ...
      const session: any = await StartStudySession(plan.id, subject);
      if (session) {
        // Notify success
